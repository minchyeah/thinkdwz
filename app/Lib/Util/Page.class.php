...
class Page {
	...
	public function render()
	{
		...
		if ($upRow>0){
			$this->prePage = str_replace('__PAGE__',$upRow,$url);
			$upPage	 = str_replace(array('__URL__', '__TITLE__'), array(str_replace('__PAGE__',$upRow,$url), $this->config['prev']), $this->prevPageHtml);
		}else{
			$upPage	 =   '';
		}
		...
		if ($downRow <= $this->totalPages){
			$this->nextPage = str_replace('__PAGE__',$downRow,$url);
			$downPage	 = str_replace(array('__URL__', '__TITLE__'), array(str_replace('__PAGE__',$downRow,$url), $this->config['next']), $this->nextPageHtml);
		}else{
			$downPage   =   '';
		}
		// << < > >>
		...
		for($i; $i<=$j; $i++){
			...
			if($page!=$this->nowPage){
				if($page<=$this->totalPages){
					$linkPage .= str_replace(array('__URL__', '__TITLE__'), array(str_replace('__PAGE__',$page,$url), $page), $this->linkPageHtml);
				}else{
					break;
				}
			}else{
				if($this->totalPages != 1){
					$linkPage .= str_replace('__TITLE__', $page, $this->currentPageHtml);
				}
			}
		}
		$pageStr	 =   str_replace(
